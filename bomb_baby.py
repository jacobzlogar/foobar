...
def answer(M, F):
    ...
    if mach > facula:
        diff = mach - facula
        count = int(round(diff/facula))
        mach = mach - facula * count
        result = reverse(count, mach, facula)

    elif facula > mach:
        diff = facula - mach
        count = int(round(diff/mach))
        facula = facula - mach * count
        result = reverse(count, mach, facula)
...
def reverse(count, m, f):
    if m == 1 and f == 1:
        return str(count)
    if is_valid(m, f):
        if m > f:
            m = m - f
            return reverse(count + 1, m, f)
        elif f > m:
            f = f - m
            return reverse(count + 1, m, f)
